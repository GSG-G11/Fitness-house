...
describe('Gyms API', () => {
  ...
  test('Gyms Filter - GET -/api/v1/gyms/filter?name=ا&city=غزة&typeGender=mail&minPrice=10&maxPrice=150&availability=true&page=1&features=ميدان%20تنافسي&review=1', async () => {
    const response = await request(app)
      .get(
    ...
    expect(response.body.pages.pageSize).toBe(3);
    expect(response.body.pages.totalItems).toBe(1);
    expect(response.body.pages.currentPage).toBe(1);
  });
});
afterAll(() => {
  connection.close();
